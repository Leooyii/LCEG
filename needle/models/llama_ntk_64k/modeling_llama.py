...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        if seq_len > 8192:
            base = self.base * (
            (self.scaling_factor * seq_len / self.max_position_embeddings) * (self.scaling_factor * self.max_position_embeddings / seq_len) - (self.scaling_factor - 1)
            ) ** (self.dim / (self.dim - 2))
        else:
            base = self.base * (
            (self.scaling_factor) * (self.scaling_factor / 2) - (self.scaling_factor - 1)
            ) ** (self.dim / (self.dim - 2))
